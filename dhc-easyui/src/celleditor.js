export default (function($){ 
	...
	function getCurrCell(target){
		...
		if (cell.length){
			return {
				index: cell.closest('tr.datagrid-row').attr('datagrid-row-index'),
	...
	function gotoCell(target, p){
		...
		if (p == 'up' && rowIndex > 0){
			...
			rowIndex++;
		} else if (p == 'left' && colIndex > 0){
			colIndex--;
		} else if (p == 'right' && colIndex < fields.length-1){
			colIndex++;
		}
	...
	function editCell(target, param){
		...
		//保存当前编辑列的信息，方便在onBeforeEdit事件中判断
		dg.data("_editField",param.field);
