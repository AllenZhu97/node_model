	...
	function create(target){
		...
		$(target).combo($.extend({}, opts, {
			onShowPanel: function(){
				...
				//update : tianming 在可选现前追加搜索框
				input.val("");
				input.width($(target).combo('panel').width()-8);
			}
		}));

		//update : tianming 在可选现前追加搜索框
		var input = $("<input type='text' id=input_"+opts._id+" class='f-search' />");
		input.keyup(function(e){
			switch(e.keyCode){
				...
				default:
					var q = input.val();
					doQuery(target,q,false);
			}
		});
		input.wrap("<div  style='background-color:#fff;'></div>");
		//解决因opts.name获取不到，导致有的下拉框追加不到搜索框，现改成opts._id方式获取 20190624 lpt
		if($("#input_"+opts._id).length<=0){
			$(target).combo('panel').before(input);
		}
