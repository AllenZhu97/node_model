...
(function($){
	...
	function init(target){
		...
		var span = $(
				...
				'<input type="hidden" class="textbox-value" initValue="" >' +
				'</span>'
				).insertAfter(target);
	...
	function buildTextBox(target){
		...
		$('#'+inputId).attr('tabindex', $(target).attr('tabindex')||'').css('text-align', $(target).css('text-align'));
	...
	$.fn.textbox.methods = {
		...
		// getInitValue: function(jq){
		// 	return jq.each(function(){
		// 		var state = $.data(this, 'textbox');
		// 		return state.textbox.find('.textbox-value').val();
		// 	});
		// },
