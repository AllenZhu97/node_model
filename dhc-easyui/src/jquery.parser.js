(function($){
	...
	$.parser = {
						...
						r.each(function(){
							...
							if(opts.data && opts.data.instance && 'AMS' == opts.data.instance.sysType){
								...
								if(dataOptions.instance.el==undefined){
									dataOptions.instance.el = {};
								}
								if(opts&&this.id){
									dataOptions.instance.el[this.id] = $(this);
								}
								$(this).data("instance",opts?dataOptions.instance:null);//设置当前组件所属业务组件的命名空间
							}else{
								dataOptions.instance = opts.instance;
								//将组件的jquery对象绑定到instance的el属性上
								if(opts.instance.el==undefined){
									opts.instance.el = {};
								}
								if(opts&&this.id){
									opts.instance.el[this.id] = $(this);
								}
								$(this).data("instance",opts?opts.instance:null);//设置当前组件所属业务组件的命名空间
							}
							$(this)[name](dataOptions);
						});
					} else {
		...
		parseOptions: function(target, properties){
			...
			var tagName = t.context.tagName;
			var instance = t.data("instance");//取得组件的命名空间
			//这里获取当前处理组件的标签名，若是表格列特殊处理
			if (s || tagName == 'TH'){
				if((tagName == 'TH' )){
					instance = t[0].instance;
				}
				if(instance!=undefined&&instance!=null){
					var props = s.split(",");
					var reg=new RegExp("^on");
					for(var i = 0;i<props.length;i++){
						var temp = props[i].split(":");
						if(temp[1]!=''&&temp[1]!=null){
							//操作列需要特殊处理的在if中处理
							if(s.indexOf('optCol')>0){
								if (temp[0]=='onBeforeDisplay' || temp[0]=='handler' ){
									if(!(temp[1].indexOf("\'") >=0 || temp[1].indexOf("\"")>=0)){
										props[i] = temp[0]+':"'+temp[1]+'"';
									}
								}
							}else{
								//事件
								// 避免将switchbutton组件中的onText和offText属性匹配为方法
								if((reg.test(temp[0]) && (temp[0] !='onText' && temp[0] !='offText')) || temp[0]=='handler'){
									props[i] = temp[0]+":function(){var target = $(this).data('instance'); return target."+temp[1]+".apply(target,arguments)}";
								}
								//方法
								if(temp[0]=='selectAfter' ||
								   temp[0]=='beforeOpen' || temp[0]=='beforeSubmit' || temp[0]=='ajaxSuccess' || temp[0]=='before' || temp[0]=='onSubmit'){
									props[i] = temp[0]+':function(){var target = $("#' + target.id + '","#'+instance.namespace+'").data("instance"); return target.'+temp[1]+'.apply(target,arguments)}';
								}
								//表格行方法
								if(temp[0]=='getSelectDataByJs' || temp[0]=='formatter' ){
									let methodName = temp[1].replace(/\"/g,''); // 去掉方法名字两边的双引号(有的带有的不带)，要不会报错  add by heyh 2019-12-20
									props[i] = temp[0]+':function(){var target = $("#' + target.parentElement.parentElement.parentElement.id + '","#'+instance.namespace+'").data("instance"); return target.'+methodName+'.apply(target,arguments)}';
								} 
							}
						}
					}
	...
	$.fn._size = function(options, parent){
		...
		function _setSize(target, property, parent, options){
			...
			/**
			 * 解决当组件为放大镜时，产生的样式错误问题。
			 */
			var className = t.attr("class");
			if ( className == "textbox easyui-fluid combo"){
				if (p == "height"){
					val = 24
				}
				if (p == "width"){
