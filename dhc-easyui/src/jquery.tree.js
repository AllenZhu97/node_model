...
//		param = param || {};
  ...
  /**
	 ...
	 * caolele 20190128
	 */
	function unSelectNode(target, nodeEl){
		var opts = $.data(target, 'tree').options;
		var node = getNode(target, nodeEl);
		if (opts.onBeforeSelect.call(target, node) == false) return;
		$(target).find('div.tree-node-selected').removeClass('tree-node-selected');
	...
	$.fn.tree.methods = {
		...
		findBy: function(jq, args){
			return findNodeBy(jq[0], args[0], args[1]);
		},
    ...
    unselect:function(jq, nodeEl){
			return jq.each(function(){
				unSelectNode(this, nodeEl);
			});
		},
			...
			function getTreeData(depth, children){
				...
				for(var i=0; i<children.length; i++){
					...
					if(item.id == undefined){
						// 账户管理系统 加载树型数据没有设置id属性，所以放在这个地方统一处理 add by heyh 2019-12-25
						item.id=item[opts.idField];
					}
					cc.push('<li>');
					cc.push('<div id="' + item.domId + '" class="tree tree-node">');
				...
				for (i = 0, l = data.length; i < l; i++) {
					if (tmpMap[data[i][parentField]] && data[i][idFiled] != data[i][parentField]) {
						if (!tmpMap[data[i][parentField]]['children']){
							...
							tmpMap[data[i][parentField]].state = isOpen ? 'open':'closed';
						}
						...
						data[i]['id'] = data[i][idFiled];
						data[i]['text'] = data[i][textFiled];
						if(opt.isAsync&&!data[i]['children']){
							data[i]['children'] = [];
							data[i].state = isOpen ? 'open':'closed';
						}
