 ...
 * 
 */
import validte from './validate.js';
(function($){
	...
	function validate(target){
		...
		function doValidate(vtype, vparam){
			...
			if (rule && value){
				...
				if (!rule['validator'].call(target, value, param,opts)){
					var message = rule['message'];
					if(typeof(message)=='function'){//update : 如果提示信息是个方法，则调用该方法返回提示信息   ， tianming 20160121
						message = message.call(target, value, param,opts);
					}
					if (param){
		...
		function _validate(){
			...
			if(target.localName!='textarea'){//目前只有工作流处理说明用了testarea,暂用此作过滤  taojinsong 20160829
				var ispass=$("input[name='workflow_result']");//当不通过时不做处理 taojinsong 20160816
				if(!!ispass && ispass.val()=='unpass'){
					return true;

				}
			}
			if (opts.required){
				if ($.trim(opts.val(target)) == ''){// update : 校验非空时去除首位空格  tianming 20160426
	...
	$.fn.validatebox.defaults = {
		...
		events: {
			//focus: focusEventHandler,  - update : tianming
