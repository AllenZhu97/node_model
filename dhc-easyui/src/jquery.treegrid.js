...
//		if (!opts.url) return;
	...
	function getParent(target, idValue){
		...
		if (row._parentId){
			...
			if(row[opts.parentField] && row[opts.parentField] != idValue){ //  !=idValue为了排除顶层父节点，防止出现死循环
					return find(target,  row[opts.parentField]);
			}else{
					return null;
			}
		} else {
			if (row.up_no_val){
				return find(target, row.up_no_val);
			} else {
				return null;
			}
		}
...
//							val = row[field] || '&nbsp;';
	...
	$.fn.treegrid.defaults = $.extend({}, $.fn.datagrid.defaults, {
		...
		loadFilter: function(ipage, parentId){
			// update by heyuhong 20170324   修改前 var data = ipage.rows; 修改原因 ipage没有rows属性会导致程序报错
			var data ={};
			if("rows" in ipage){
				data = ipage.rows;
			}else if("rows" in ipage[0] ){
				data = ipage[0].rows;
			}
