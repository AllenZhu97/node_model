	...
	function buildGrid(target){
		...
		if (opts.toolbar) {
			if ($.isArray(opts.toolbar)){
				...
				var toolbarid = "";
				if(opts.toolbarid){
					toolbarid = " id='"+opts.toolbarid+"'";
				}
				var tb = $('<div '+toolbarid+' class="datagrid-toolbar"><table cellspacing="0" cellpadding="0"><tr></tr></table></div>').prependTo(panel);
				var tr = tb.find('tr');
		...
		if (opts.pagination) {
			...
			pager.pagination({
				//解决datagird初始化，在没有数据的情况下总条数显示错误  caolele 20170927
				total:0,
...
//				}
	...
	function fixMergedSize(target){
		...
		dc.view.find('td.datagrid-td-merged').each(function(){
			...
			//设置列宽时，如果为空则跳过 update by yexinghai 20190306
		...
		if (!opts.remoteSort && opts.sortName){
			...
			data.rows.sort(function(r1,r2){
				...
				for(var i=0; i<names.length; i++){
					...
					r = sortFunc(r1[sn], r2[sn],{up:r1,next:r2}) * (so=='asc'?1:-1);
					if (r != 0){
						return r;
					}
				}
...
//		var bck = opts.finder.getTr(target, '', 'allbody').find('div.datagrid-cell-check input[type=checkbox]');
	...
	function endEdit(target, index, cancel){
		...
		$(target).datagrid('refreshRow', index,{type:"edit",disp:true});
		...
		tr.children('td').each(function(){
			...
			if (col && col.editor){
				...
				if (editor){
					...
					if(col.getSelectDataByJs!=undefined){
						editoptions.data = col.getSelectDataByJs(index);
					}
...
//			endEdit(target, i, true);
	...
	function mergeCells(target, param){
		...
		//当行或列为1行时也合并单元格，解决删除合并后的单元格，当为最后一列时行错位的问题 modify by zhangjibing
		...
		firstTd.attr('rowspan', param.rowspan-count)
		fixMergedSize(target);
	...
	$.fn.datagrid = function(options, param,obj){
		if (typeof options == 'string'){
			return $.fn.datagrid.methods[options](this, param, obj);
		}
		...
		return this.each(function(){
			...
			if(opts.delayLoad==undefined||!opts.delayLoad){
				request(this);
			}
		});
	};
	...
	var editors = $.extend({},
		getDefaultEditors([
			...
			'timespinner','datetimespinner','textReadonly','combotree_single'
		]), {
			textarea: {
				init: function(container, options){
			...
			textReadonly: {
				init: function (container, options) {
					var input = $('<input type="button" readonly="readonly" class="datagrid-editable-input" onclick="button()">').appendTo(container);
					return input;
				},
				getValue: function (target) {
					return $(target).val();
			...
			combotree_single: {
				init: function (container, options) {
					var box = $('<input type="text" class="datagrid-editable-input">').appendTo(container);
...
//						msg.css('marginLeft', -msg.outerWidth()/2);
		...
		refreshRow: function(jq, index,obj){
			return jq.each(function(){
				var opts = $.data(this, 'datagrid').options;
				opts.view.refreshRow.call(opts.view, this, index,obj);
			});
		},
		validateRow: function(jq, index){
			return validateRow(jq[0], index);
		},
		updateRow: function(jq, param,obj){	// param: {index:1,row:{code:'code1',name:'name1'}}
			return jq.each(function(){
				var target = this;
                    var state = $.data(target, 'datagrid');
                    var opts = state.options;
	...
	var defaultView = {
		...
		renderRow: function(target, fields, frozen, rowIndex, rowData,obj){
			var opts = $.data(target, 'datagrid').options;
			...
			for(var i=0; i<fields.length; i++){
				...
				if (col){
					...
					var valueTemp = "";
					if(col.formatter){
						valueTemp  = col.formatter(value, rowData, rowIndex,obj,target.id);
						// 如果格式化内容不存在“<” 和">",则显示格式化内容，排除 格式化内容是连接、图片、input控件等等
						if(typeof valueTemp == 'string' && valueTemp.indexOf("<") == valueTemp.indexOf(">")){
							title = valueTemp;
						} else { 
							// 当格式化内容中含有链接、图片等等时，修改悬浮显示 update by zhuxingpeng 20200328
							if(rowData[field+"_zh"]){
									title = rowData[field+"_zh"];
							}
						}
		...
		refreshRow: function(target, rowIndex,obj){
			this.updateRow.call(this, target, rowIndex, {},obj);
		},

		updateRow: function(target, rowIndex, row,obj){
			var opts = $.data(target, 'datagrid').options;
			var rows = $(target).datagrid('getRows');
			...
			function _update(frozen){
				...
				var tdHidden = $("td:hidden",tr);
				var rowspanTd = $("td[rowspan]",tr);
				var rowspanArray = [];
				$.each(rowspanTd,function(index,ele){
					var targ = $(ele);
					var rowspans = targ.attr("rowspan");
					var idx = targ.index();
					rowspanArray.push({index:idx,count:rowspans});
				});
				var colspanTd = $("td[colspan]",tr);
				var fieldArray = [];
				for(var index =0;index< tdHidden.length;index++){
					var field = $(tdHidden.get(index)).attr("field");
					if(field!=undefined){
						fieldArray.push(field);
					}
				}
				tr.html(this.renderRow.call(this, target, fields, frozen, rowIndex, rows[rowIndex],obj));
				tr.attr('style', styleValue).removeClass(oldClassValue).addClass(classValue);
				if(obj!=undefined && obj!=null && obj.disp!=false){
					// 用for遍历数组，for-in 遍历数组元素，还会遍历自定义属性； modify by heyh 2019-12-24
					for(var i = 0,len=fieldArray.length; i < len; i++) {
						$("td[field='"+fieldArray[i]+"']",tr).hide();
					}
					for(var i = 0,len=rowspanArray.length; i < len; i++) {
						$("td:eq("+rowspanArray[i].index+")",tr).attr("rowspan",rowspanArray[i].count);
					}
				}
				if (checked){
					tr.find('div.datagrid-cell-check input[type=checkbox]')._propAttr('checked', true);
	...
	$.fn.datagrid.defaults = $.extend({}, $.fn.panel.defaults, {
		...
		loader: function(param, success, error){
			...
			$.ajax({
				...
				url: opts.url,
				data: param,
				async:opts.async==undefined||opts.async==true?true:false,
				dataType: 'json',
...
(function ($) {
  ...
  $.extend($.fn.datagrid.methods, {
    clientPaging: function (jq, data, field) {
      return jq.each(function () {
        var dg = $(this);
        var state = dg.data('datagrid');
        state.data = data;
    ...
    insertRow: function (jq, data) {
      return jq.each(function () {
        insertRowMethod.call($.fn.datagrid.methods, $(this), data);
        var state = $(this).data('datagrid');
        if (state.options.loadFilter == pagerFilter) {
          state.allRows[data.index] = data.row;
          $(this).datagrid('loadData', state.allRows);
        }
      });
    },
    ...
    /**
     * add by yexinghai 20190104
     * 第二列合并单元格，是在前一列的合并后的基础上进行合并
     */
    autoMergeCell: function (jq, fields) {
      return jq.each(function () {
        var target = $(this);
        var Arr = fields;
        var fldName;
        ...
        for (i = length; i >= 0; i--) {
          ...
          if (i == 1) {
            for (row = 0; row <= RowCount; row++) {
              if (row == RowCount) {
                ...
                CurValue = target.datagrid("getRows")[row][fldName];
              }
          ...
          } else { //第二列合并单元格是在第一列的基础上进行合并
            for (var a = 0; a < rowarr.length; a++) {
              var rowlength = rowarr[a + 1] - rowarr[a];
              var rowindex = rowarr[a];
              var init = 0;
              for (var row = 0; row <= rowlength; row++) {
                if (row == rowlength) {
                  CurValue = "";
                } else if (a != 0) {
                  CurValue = target.datagrid("getRows")[row + rowindex][fldName];
                } else {
                  CurValue = target.datagrid("getRows")[row][fldName];
                }
                if (PerValue == CurValue) {
