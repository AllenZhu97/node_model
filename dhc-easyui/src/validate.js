...
export default {
  ...
  isIDCardNo: {
    validator: function (num, p) {//验证身份证格式 
      ...
      if (len == 15) {
        ...
        if (isNaN(num.substring(0, 16))) { return false; }
      ...
      if (a != null) {
        if (len == 15) {
          ...
          var idcard_array = num.split("");

          //校验第18位
          var s =
            parseInt(idcard_array[0]) * 7 + parseInt(idcard_array[1]) * 9
            + parseInt(idcard_array[2]) * 10 + parseInt(idcard_array[3]) * 5
            + parseInt(idcard_array[4]) * 8 + parseInt(idcard_array[5]) * 4
            + parseInt(idcard_array[6]) * 2 + parseInt(idcard_array[7]) * 1
            + parseInt(idcard_array[8]) * 6 + parseInt(idcard_array[9]) * 3
            + parseInt(idcard_array[10]) * 7 + parseInt(idcard_array[11]) * 9
            + parseInt(idcard_array[12]) * 10 + parseInt(idcard_array[13]) * 5
            + parseInt(idcard_array[14]) * 8 + parseInt(idcard_array[15]) * 4
            + parseInt(idcard_array[16]) * 2;

          var validate = new Array('1', '0', 'X', '9', '8', '7', '6', '5', '4', '3', '2');
          s = s % 11;
          B = B && (a[7] == validate[s])
        }
        if (!B) {
      ...
      if (p != undefined && (p == 'y' || p == 'Y')) {//远程校验	
        var returnFlag = "";
        jQuery.ajax({
          url: basePath + "financemanage/validateIDCardNo.do",
          async: false,
          data: { idcardno: num },
